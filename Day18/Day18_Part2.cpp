...
int main() {
  ...
  // Part 2: Initialize the corners to 'on' since they're stuck on, but not on the dummy extra rows/column
  grid[1][1] = 1;                                       // Top left
  grid[1][EXTENDED_GRID_SIZE-2] = 1;                    // Top right
  grid[EXTENDED_GRID_SIZE-2][1] = 1;                    // Bottom left
  grid[EXTENDED_GRID_SIZE-2][EXTENDED_GRID_SIZE-2] = 1; // Bottom right
  ...
  for(int z = 0; z < NUM_STEPS; ++z) {
    ...
    // Replace grid with newGrid, sans the dummy rows/columns
    for(int row = 1; row < EXTENDED_GRID_SIZE-1; ++row) {
      for(int column = 1; column < EXTENDED_GRID_SIZE-1; ++column) {
    ...
    // Part 2: Overwrite the corners to 'on' since they're stuck on, but not on the dummy extra rows/column
    grid[1][1] = 1;                                       // Top left
    grid[1][EXTENDED_GRID_SIZE-2] = 1;                    // Top right
    grid[EXTENDED_GRID_SIZE-2][1] = 1;                    // Bottom left
    grid[EXTENDED_GRID_SIZE-2][EXTENDED_GRID_SIZE-2] = 1; // Bottom right
